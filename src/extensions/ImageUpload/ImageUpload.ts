...
export const ImageUpload = Node.create<ImageOptions>({
  ...
  addProseMirrorPlugins() {
    ...
    const handleNewImageFiles = (files: File[], insertPosition?: number): void => {
      ...
      // Modifica per utilizzare l'interfaccia di caricamento
      if (options.upload && typeof options.upload === 'function') {
        ...
        // Basta semplicemente convertirlo in base64 TODO e caricarlo sul server
    ...
    return [
      new Plugin({
        props: {
          ...
          /**
           * Consenti il ​​trascinamento delle immagini
           * @param _
           * @param event
           * @returns
           */
