...
export interface CreateSkeletonPayload extends GridSize {
  withHeaderRow: boolean
}

interface Emits {
  (event: 'create-skeleton', payload: CreateSkeletonPayload): void
}
...
function onMouseDown(rows: number, cols: number) {
  emit('create-skeleton', { rows, cols, withHeaderRow: unref(withHeaderRow) })
  resetTableGridSize()
}
...
<template>
  <Popover>
    <PopoverTrigger as-child>
      <button :disabled="disabled" class="p-0 m-0">
        <slot name="trigger" />
      </button>
    </PopoverTrigger>
    <PopoverContent class="w-full" align="start" side="bottom">
      <div content-class="table-grid-size-editor" content-style="padding:0">
        ...
        <div class="mt-2 text-sm text-center text-zinc-600">
